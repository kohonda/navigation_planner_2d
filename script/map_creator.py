...
def initialize_map(height, width, type="indoor"):
    if type == "outdoor":
        ...
        margin = 3 # If the wall is too thin, pymap2d's lidar scan simulation may show through
            ...
            if no_overlap:
                if len(rooms_list) == 0:
                    try:
                        ...
                        print("Room "+str(room)+" created at position " +
                    ...
                    for room_node in rooms_list:
                        ...
                        if np.linalg.norm(np.array(node)-np.array(room_node)) < distance:
                            if (i == len(tree)-1) and (room > rooms_created):
                                print("No valid position for room " +
                                      str(room)+" found.")
                            collision = True
                            break
                    if collision:
...
def create_simplest_map(height: float, width: float, obstacle_positions: list, obstacle_sizes: list):
    map = initialize_map(height, width, type="outdoor")
    for position in obstacle_positions:
        pos = specify(map=map, position=position)
        map[slice(pos[0]-obstacle_sizes[obstacle_positions.index(position)][0], pos[0]+obstacle_sizes[obstacle_positions.index(position)][1]+1),  # create 1 pixel obstacles with extra radius if specified
            slice(pos[1]-obstacle_sizes[obstacle_positions.index(position)][1], pos[1]+obstacle_sizes[obstacle_positions.index(position)][1]+1)] = 1
    return map
...
def generate_random_maps(map_config_path: str, random_map_num: int, seed: int):
    ...
    for i in range(indoor_map_num):
        ...
        indoor_map = create_indoor_map(
                height, width, corridor_radius, iterations, room_number, room_width, room_height, no_overlap)
...
"""
    args:
        config_path: config file name in config folder
"""
