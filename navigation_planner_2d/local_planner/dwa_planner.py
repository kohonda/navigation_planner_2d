...
from navigation_planner_2d.local_planner import LocalPlannerBase, LocalPlannerOutput
from navigation_simulator_2d.utils import MapHandler, ParameterHandler
from navigation_simulator_2d.common import RobotCommand, AgentState

class DWAPlanner(LocalPlannerBase):
    # common settings
    CONTROL_INTERVAL_TIME: float # [s]
    ROBOT_RADIUS: float # [m]
    # Sampling parameters
    PREDICTION_INTERVAL : float = 0.1 # [s]
    PREDICTION_STEP : int = 10 # number of steps for the prediction; prediction horizon is PREDICTION_STEP * PREDICTION_INTERVAL
    LINEAR_VEL_X_RESOLUTION : int = 5
    LINEAR_VEL_Y_RESOLUTION : int = 3
    ANGULAR_VEL_RESOLUTION : int = 8
    # Dynamics limitations
    LINEAR_VEL_X_LIMIT : dict = {'MAX': 2.0, 'MIN': 0.0} # [m/s]
    LINEAR_VEL_Y_LIMIT : dict = {'MAX': 0.1, 'MIN': -0.1}
    ANGULAR_VEL_LIMIT : dict = {'MAX': 2.0, 'MIN': -2.0} # [rad/s]
    ACCEL_LIMIT : list = [2.5, 2.5, 4.0] # ax[m/s^2], ay[m/s^2], dw[rad/s^2]
    # Weights for the cost function
    PATH_DISTANCE_BIAS : float = 0.1 # [1/m] : 軌道終端からグローバルパスまでへの距離に対する重み
    ...
    ANGLE_TOLERANCE : float = np.pi / 180.0 * 360.0 # [rad]
    # Sub-goal search
    SUB_GOAL_AHEAD : float = 3.0 # [s]
    SUB_GOAL_AHEAD_OFFSET : float = 1.0 # [m]
    # map params
    INFLATION_RADIUS : float # = robot radius
    ...
    def __init__(self, params: ParameterHandler) -> None:
        ...
        self.INFLATION_RADIUS = params.robot_radius
        
        if params.dwa_config is not None:
            self.INFLATION_RADIUS = params.dwa_config['inflation_radius']
            ...
            self.COLLISION_SCALE = params.dwa_config['collision_scale']
        else:
    ...
    def set_costmap(self, costmap: MapHandler) -> None:
        self._local_cost_map = costmap.get_inflated_map(inflation_radius=self.INFLATION_RADIUS)
    ...
    def is_goal_reached(self, robot_obs: RobotObservation, goal_state: AgentState) -> bool:
        ...
        if dist_to_goal < self.POSITION_TOLERANCE and angle_to_goal < self.ANGLE_TOLERANCE:
            is_goal = True
    ...
    def _search_sub_goal(self, current_pos: np.ndarray, current_vx: float, reference_path: np.ndarray) -> int:
        """
    ...
    def compute_velocity_command(self, robot_obs: RobotObservation, goal_state: AgentState, reference_path: np.ndarray)->LocalPlannerOutput:
        ...
        if is_goal_reached or reference_path is None:
            zero_command = RobotCommand(linear_vel=np.array([0.0, 0.0]), angular_vel=0.0)
            return LocalPlannerOutput(control_command=zero_command, is_goal_reached=True, predict_path_list=None, best_index=None, path_obs_costs=None, sub_goal_index=None)
        
        # Search sub-goal
        sub_goal_index = self._search_sub_goal(robot_obs.state.pos, robot_obs.state.linear_vel[0] , reference_path[0])
        
        # compute dynamic window
        linear_x_dw,linear_y_dw, angular_dw = self._calc_dynamic_window(robot_obs.state)
        
        # calculate candidates
        vx_candidates = np.linspace(linear_x_dw[0], linear_x_dw[1], self.LINEAR_VEL_X_RESOLUTION)
        vy_candidates = np.linspace(linear_y_dw[0], linear_y_dw[1], self.LINEAR_VEL_Y_RESOLUTION)
        w_candidates = np.linspace(angular_dw[0], angular_dw[1], self.ANGULAR_VEL_RESOLUTION)
        
        # forward prediction
        current_pose = np.array([robot_obs.state.pos[0], robot_obs.state.pos[1] , robot_obs.state.yaw])
        current_vel = np.array([robot_obs.state.linear_vel[0], robot_obs.state.linear_vel[1], robot_obs.state.angular_vel])
        accel_limits = np.array([self.ACCEL_LIMIT[0], self.ACCEL_LIMIT[1], self.ACCEL_LIMIT[2]])
        prediction_step = int(self.PREDICTION_STEP)
        prediction_interval = float(self.PREDICTION_INTERVAL) 
        
        target_vel_array = np.array(np.meshgrid(vx_candidates, vy_candidates, w_candidates)).T.reshape(-1,3)
        predict_traj_array = self._generate_trajectory_array(current_pose, current_vel, target_vel_array, accel_limits, prediction_step, prediction_interval)
                    
        # evaluate trajectory and find best trajectory
        costs, obs_cost = self._eval_all_trajectories(predict_traj_array, goal_state, reference_path, sub_goal_index)
        ...
        return LocalPlannerOutput(control_command=robot_command, predict_path_list=predict_path_list, best_index=best_index, is_goal_reached=is_goal_reached, path_obs_costs=obs_cost, sub_goal_index=sub_goal_index)
    ...
    def _calc_dynamic_window(self, robot_state: AgentState)->Tuple[list,list, list]:
        ...
        linear_x_window_s = [self.LINEAR_VEL_X_LIMIT['MIN'], self.LINEAR_VEL_X_LIMIT['MAX']]
        linear_y_window_s = [self.LINEAR_VEL_Y_LIMIT['MIN'], self.LINEAR_VEL_Y_LIMIT['MAX']]
        angular_window_s = [self.ANGULAR_VEL_LIMIT['MIN'], self.ANGULAR_VEL_LIMIT['MAX']]
                
        # Dynamic window from robot motion
        linear_x_window_d = [robot_state.linear_vel[0] - self.ACCEL_LIMIT[0] * self.CONTROL_INTERVAL_TIME, robot_state.linear_vel[0] + self.ACCEL_LIMIT[0] * self.CONTROL_INTERVAL_TIME]
        linear_y_window_d = [robot_state.linear_vel[1] - self.ACCEL_LIMIT[1] * self.CONTROL_INTERVAL_TIME, robot_state.linear_vel[1] + self.ACCEL_LIMIT[1] * self.CONTROL_INTERVAL_TIME]
        angular_window_d = [robot_state.angular_vel - self.ACCEL_LIMIT[2] * self.CONTROL_INTERVAL_TIME, robot_state.angular_vel + self.ACCEL_LIMIT[2] * self.CONTROL_INTERVAL_TIME]
        
        # calculate dynamic window
        linear_x_dynamic_window = [max(linear_x_window_s[0], linear_x_window_d[0]), min(linear_x_window_s[1], linear_x_window_d[1])]
        linear_y_dynamic_window = [max(linear_y_window_s[0], linear_y_window_d[0]), min(linear_y_window_s[1], linear_y_window_d[1])]
        angular_dynamic_window = [max(angular_window_s[0], angular_window_d[0]), min(angular_window_s[1], angular_window_d[1])]
  
        return linear_x_dynamic_window, linear_y_dynamic_window , angular_dynamic_window
    
    @staticmethod
    @numba.jit(nopython=True, cache=True, fastmath=True)
    def _generate_trajectory_array(pose: np.ndarray, 
                                    vel: np.ndarray, 
                                    target_vel_array: np.ndarray, 
                                    accel_limits: np.ndarray, 
                                    prediction_step: int, 
                                    prediction_interval: float)->np.ndarray:
        """
        ...
        predict_traj_array : np.ndarray = np.zeros((target_vel_array.shape[0], prediction_step, 6))
           
        for i in range(target_vel_array.shape[0]):
            predict_traj_array[i, :] = generate_trajectory(pose, vel, target_vel_array[i], accel_limits, prediction_step, prediction_interval)
        
        return predict_traj_array
    
    def _eval_trajectory(self, traj: np.ndarray, goal_state: AgentState, reference_path: np.ndarray, sub_goal_index :int) -> float:
        """
        ...
        cost += self.SUB_GOAL_DISTANCE_BIAS * math.sqrt(dx_to_sub_goal**2 + dy_to_sub_goal**2)
        
        # eval angle to sub goal
        _angle_to_sub_goal = np.arctan2(dy_to_sub_goal, dx_to_sub_goal) - traj_angle
        angle_to_sub_goal = np.arctan2(np.sin(_angle_to_sub_goal), np.cos(_angle_to_sub_goal))
        ...
        dx_reference_path = reference_path[0, next_index, 0] - reference_path[0, nearest_idx_of_terminal, 0]
        dy_reference_path = reference_path[0, next_index, 1] - reference_path[0, nearest_idx_of_terminal, 1]
        ...
        # eval collision
        obs_cost, _ = self._calc_obs_cost(traj[:,0:2])
        cost += self.COLLISION_SCALE * obs_cost
        
        return cost
        
    
    def _eval_all_trajectories(self, trajs: np.ndarray, goal_state: AgentState, reference_path: np.ndarray, sub_goal_index: int) -> np.ndarray:
        """
        ...
        for i in range(trajs.shape[0]):
            costs[i] = self._eval_trajectory(trajs[i], goal_state, reference_path, sub_goal_index)
