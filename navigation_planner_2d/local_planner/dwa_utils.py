...
def compute_new_vel(current_vel: np.ndarray, target_vel: np.ndarray, acc_limits: np.ndarray, dt: float) -> np.ndarray:
    """
...
def motion_model(current_pose: np.ndarray, vel: np.ndarray, dt: float) -> np.ndarray:
    ...
    """
    xy = current_pose[0:2]
    rot_mat = np.array([[np.cos(current_pose[2]), -np.sin(current_pose[2])], [np.sin(current_pose[2]), np.cos(current_pose[2])]])
...
@numba.jit(nopython=True, cache=True, fastmath=True)
def generate_trajectory(pose: np.ndarray, 
                            vel: np.ndarray, 
    ...
    for i in range(prediction_step):
        predict_vel = compute_new_vel(predict_vel, target_vel, accel_limits, prediction_interval)
        predict_pose = motion_model(predict_pose, predict_vel, prediction_interval)
        traj[i, :] = np.array([predict_pose[0], predict_pose[1], predict_pose[2], predict_vel[0], predict_vel[1], predict_vel[2]])
