...
def state_function(state: np.ndarray, input: np.ndarray, dt: float) -> np.ndarray:
    ...
    xy = state[0:2]
    rot_mat = np.array([[np.cos(state[2]), -np.sin(state[2])], [np.sin(state[2]), np.cos(state[2])]])
...
def cost_function(predicted_state: np.ndarray, 
                  input_series: np.ndarray, 
                  goal_state: np.ndarray,
                  reference_path: np.ndarray, 
                  dt: float, 
                  state_weights: dict, 
                  input_weights: dict) -> float:
    """
    Cost function
    ...
    for i in range(1, predicted_state.shape[0]-1):
        ...
        if index is not None and index > 0:
            cost += state_weights["dist_to_path"] * np.linalg.norm(predicted_state[i, 0:2] - reference_path[index])
...
def constraint(predicted_state: np.ndarray, 
               input_series: np.ndarray,
               cost_map: MapHandler, 
               dt: float) -> int:
    """
...
class RandomizedMPCPlanner(LocalPlannerBase):
    ...
    def __init__(self, params: ParameterHandler, 
                 ...
                 constraint: Callable = constraint) -> None:
        super().__init__()
        
        ######### Parameters #########
        self._robot_radius = params.robot_radius
        ...
        self._vx_limit : dict = {'max': 2.0, 'min': 0.0} # [m/s]
    ...
    def is_goal_reached(self, robot_obs: RobotObservation, goal_state: AgentState) -> bool:
        ...
        if dist_to_goal < self._position_tolerance and angle_to_goal < self._angle_tolerance:
            is_goal = True
    ...
    def compute_velocity_command(self, robot_obs: RobotObservation, goal_state: AgentState, reference_path: np.ndarray) -> LocalPlannerOutput:
        ...
        min_value = np.array([self._vx_limit['min'], self._vy_limit['min'], self._vw_limit['min']])
        max_value = np.array([self._vx_limit['max'], self._vy_limit['max'], self._vw_limit['max']])
        random_inputs = self._generate_random_solutions(min_value=min_value, 
                                                       max_value=max_value, 
        ...
        def state_func_batch(state_batch: np.ndarray, input_batch: np.ndarray, dt: float) -> np.ndarray:
            return np.array([self._state_function(state=state, 
                                                  input=input, 
                                                  dt=dt) for state, input in zip(state_batch, input_batch)])
        
        initial_state = np.array([robot_obs.state.pos[0], robot_obs.state.pos[1], robot_obs.state.yaw])
        predicted_states = self._predict(state_func_batch = state_func_batch,
                                         initial_state = initial_state,
                                         input_vectors = random_inputs,
                                         dt = self._control_interval_time)
        
        # Check constraints for all solution candidates
        def constraint_func_batch(state_batch: np.ndarray, input_batch: np.ndarray) -> np.ndarray:
            return np.array([self._constraint(predicted_state=state, 
                                              input_series=input, 
                                              cost_map=self._local_cost_map, 
                                              dt=self._control_interval_time) for state, input in zip(state_batch, input_batch)])
        
        valid_predicted_states, valid_inputs = self._check_constraints(constraint_func=constraint_func_batch,
        ...
        if valid_predicted_states.shape[0] == 0:
            # pure rotation
            rotate_command = RobotCommand(linear_vel=np.array([0.0, 0.0]), angular_vel=max_value[2])
            return LocalPlannerOutput(control_command=rotate_command, 
                                      is_goal_reached=False, 
                                      predict_path_list=None, 
                                      best_index=None, path_obs_costs=None, sub_goal_index=sub_goal_index)
        
        # Find the best solution candidate
        
        sub_goal_pos = reference_path[0][sub_goal_index]
        def cost_func_batch(state_batch: np.ndarray, input_batch: np.ndarray) -> np.ndarray:
            return np.array([self._cost_function(predicted_state=state, 
                                                 input_series=input, 
                                                 goal_state=sub_goal_pos,
                                                 reference_path=reference_path[0], 
                                                 dt=self._control_interval_time,
                                                 state_weights=self._state_weights,
                                                 input_weights=self._input_weights) for state, input in zip(state_batch, input_batch)])
        ...
        solution = np.mean(valid_inputs[best_indices, 0, :], axis=0) # best solution is the first step input of the best candidate
        ...
        return LocalPlannerOutput(control_command=robot_cmd, is_goal_reached=False, predict_path_list=predict_path_list, best_index=best_indices[0], path_obs_costs=None, sub_goal_index=sub_goal_index)
    ...
    def _generate_random_solutions(self, min_value: np.ndarray, 
                                   max_value: np.ndarray, 
                                   num_samples: int, 
                                   num_horizon: int,
                                   num_freq: int) -> np.ndarray:
        """
        ...
        for i in range(solution_dim):
            ...
            weights =  self._rng.normal(loc=mean, scale=variance, size=(num_samples, 2 * num_freq - 2))
    ...
    def _predict(self, state_func_batch: Callable, initial_state: np.ndarray, input_vectors: np.ndarray, dt) -> np.ndarray:
        """
        ...
        for i in range(num_horizon-1):
            predicted_states[:, i+1, :] = state_func_batch(predicted_states[:, i, :], input_vectors[:, i, :], dt)
    ...
    def _check_constraints(self,
        ...
        if predicted_states.shape[0] != predicted_inputs.shape[0]:
            raise ValueError("predicted_states and predicted_inputs must have the same number of samples")
    ...
    def _eval_predicted_states(self,
    ...
    def _search_sub_goal(self, current_pos: np.ndarray, current_vx: float, reference_path: np.ndarray) -> int:
        ...
        for i in range(nearest_point_index, reference_path.shape[0]):
            if i == reference_path.shape[0] - 1:
                ...
                sub_goal_ahead = current_vx * self._sub_goal_ahead + self._sub_goal_ahead_offset
